...
# === Initialisation caméra RTSP ===
RTSP_URL = "rtsp://admin:L282777A@10.0.22.50:554/cam/realmonitor?channel=1&subtype=1&unicast=true&proto=Onvif"  # <-- Modifie ici ton URL RTSP
...
while True:
    ret, frame = cap.read()
    if not ret:
        print("[WARN] Impossible de lire l'image de la caméra RTSP.")
        break
    ...
    if elapsed >= OBSERVATION_TIME and not verdict_given and frames_analyzed >= MIN_FRAMES:
        ...
        if (eye_motion_score_ratio >= MIN_EYE_MOTION_SCORE and
            avg_spoof_pred < SPOOF_MODEL_THRESHOLD):
            verdict = "Visage reel detecte"
            verdict_reason = (f"Variations luminosite: {brightness_change_frames}/{frames_analyzed}, "
    ...
    cv2.putText(frame, f"Mouvement yeux: {eye_motion_frames}", (20, 90),
