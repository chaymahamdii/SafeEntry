...
# OUVERTURE DU FLUX AVEC IMAGEIO + FFMPEG
print("[INFO] Connexion au flux ESP32-CAM via FFmpeg...")
reader = imageio.get_reader(ESP32_CAM_STREAM_URL, format='ffmpeg')
...
try:
    for frame in reader:
        ...
        if elapsed >= OBSERVATION_TIME and not verdict_given and frames_analyzed >= MIN_FRAMES:
            ...
            verdict_reason = (f"Mouvements: {movement_frames}/{frames_analyzed}, "
                              f"Luminosité: {brightness_change_frames}/{frames_analyzed}, "
    ...
    print(f"[ERREUR] Impossible de lire le flux : {e}")
