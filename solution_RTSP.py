...
# === CONFIGURATION RTSP CAMERA ===
RTSP_CAMERA_URL = "rtsp://admin:L282777A@10.0.22.50:554/cam/realmonitor?channel=1&subtype=1&unicast=true&proto=Onvif"  
cap = cv2.VideoCapture(RTSP_CAMERA_URL, cv2.CAP_FFMPEG)  # Utilisation de FFMPEG
...
while True:
    ret, frame = cap.read()
    if not ret:
        print("[WARNING] Impossible de lire le flux vidéo.")
        continue
    ...
    if elapsed >= OBSERVATION_TIME and not verdict_given and frames_analyzed >= MIN_FRAMES:
        ...
        if (internal_motion_score >= MIN_INTERNAL_MOTION_SCORE and
            ...
            total_blinks >= 1):
            verdict = "Real face detected"
        else:
            verdict = "Spoof face detected"

        verdict_reason = (f"Movement: {movement_frames}/{frames_analyzed}, "
                          f"Brightness: {brightness_change_frames}/{frames_analyzed}, "
                          f"Texture: {internal_motion_frames}/{frames_analyzed}, "
                          f"Blinks: {total_blinks}, "
                          f"Model score: {avg_spoof_pred:.2f}")
        print(f"[RESULT] {verdict} | {verdict_reason}")
        verdict_given = True

    if verdict_given:
        color = (0, 255, 0) if "Real" in verdict else (0, 0, 255)
        cv2.putText(frame, f"{name} ({verdict})", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
        cv2.putText(frame, f"Reason: {verdict_reason}", (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
    else:
        remaining = max(0, int(OBSERVATION_TIME - elapsed))
        cv2.putText(frame, f"Observation... {remaining}s left", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)

    cv2.putText(frame, f"Internal motion: {motion_score:.0f}", (20, 90),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 200, 0), 1)
    cv2.putText(frame, f"Blinks detected: {total_blinks}", (20, 110),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 200, 0), 1)
