...
EAR_THRESHOLD = 0.25
CONSEC_FRAMES = 1
NO_FACE_RESET_DELAY = 5  # secondes à attendre avant reset quand pas de visage
...
verdict = None
verdict_reason = ""
last_face_time = None  # temps du dernier visage détecté (pour reset délai)
...
#cv2.resizeWindow("Video Stream", 1280, 720)  # ou 1920,1080 si ta vidéo est HD
...
while True:
    ...
    if len(face_locations) == 0:
        ...
        if last_face_time is None:
        ...
        elif time.time() - last_face_time > NO_FACE_RESET_DELAY:
            # Reset après délai d'absence de visage
            start_time = None
            frames_analyzed = 0
            movement_frames = 0
            brightness_change_frames = 0
            ...
            prev_face_img = None
            last_face_time = None
        # Afficher dernier verdict si existant
        if verdict_given and verdict is not None:
            color = (0, 255, 0) if "reel" in verdict.lower() else (0, 0, 255)
            cv2.putText(frame, verdict, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
            y0 = 60
            for i, line in enumerate(verdict_reason.split(", ")):
                y = y0 + i * 25
                cv2.putText(frame, line, (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 1)
        cv2.imshow("Video Stream", frame)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
        continue
    else:
        last_face_time = None  # visage détecté, reset timer absence visage
    ...
    # Ajustement échelle (car face_recognition a réduit la taille)
    top *= 2
    right *= 2
    bottom *= 2
    left *= 2
